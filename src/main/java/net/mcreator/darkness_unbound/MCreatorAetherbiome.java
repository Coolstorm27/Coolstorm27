...
public class MCreatorAetherbiome extends Elementsdarkness_unbound.ModElement {
	@ObjectHolder("darkness_unbound:aethernormal")
	public static final CustomBiome biome = null;

	public MCreatorAetherbiome(Elementsdarkness_unbound instance) {
		super(instance, 27);
	}
		...
		public CustomBiome() {
			super(new Biome.Builder()
					...
					.surfaceBuilder(
							...
							new SurfaceBuilderConfig(Blocks.GRASS_BLOCK.getDefaultState(), MCreatorAetherstone.block.getDefaultState(),
									MCreatorAetherstone.block.getDefaultState())));
			setRegistryName("aethernormal");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.GRASS_BLOCK.getDefaultState().getBlock() || ground == MCreatorAetherstone.block.getDefaultState()
							.getBlock()) && (ground2 == Blocks.GRASS_BLOCK.getDefaultState().getBlock() || ground2 == MCreatorAetherstone.block
							.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), MCreatorAetherstone.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == MCreatorAetherleaves.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, MCreatorAetherleaves.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, MCreatorAetherlog.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MCreatorAetherlog.block.getDefaultState().getBlock()
					|| blockType == MCreatorAetherleaves.block.getDefaultState().getBlock()
					|| blockType == Blocks.GRASS_BLOCK.getDefaultState().getBlock()
					|| blockType == MCreatorAetherstone.block.getDefaultState().getBlock();
		}
